...
class wallet {
  ...
  computeHash() {
    ...
    let strBlock =
      this.prevHash + this.timestamp + this.balance + JSON.stringify(this.data); // Stringify the block's data
  ...
  getWordKeys() {
    let items = [];
    let keys= [];
    fs.readFile("./JSON/items.json", "utf8", (err, jsonString) => {
      if (err) {
        console.log("File read failed:", err);
        return "failed";
      }
      let itemsJSON = JSON.parse(jsonString);
